namespace :dev do

  #Setup Development
  desc "Setup Development"
  task setup: :environment do
    images_path = Rails.root.join('public', 'system')

    puts "Configurando Ambiente de Desenvolvimento..."

    puts "Apagando Images /Public/System #{%x(rm -rf #{images_path})}"
    puts "Apagando BD... #{%x(rake db:drop)}"
    puts "Criando BD... #{%x(rake db:create)}"
    puts %x(rake db:migrate)
    puts %x(rake db:seed)
    puts %x(rake dev:generate_admins)
    puts %x(rake dev:generate_members)
    puts %x(rake dev:generate_ads)
    puts %x(rake dev:generate_comments)

    puts "Configurando Ambiente de Desenvolvimento...[OK]"
  end

  #Create Admins
  desc "Cria Administradores Fake"
  task generate_admins: :environment do
    puts "Cadastrando Administradores..."

    10.times do
      Admin.create!(
        name: Faker::Name.name,
        email: Faker::Internet.email,
        password: "123456",
        password_confirmation: "123456",
        role: [0,0,1,1,1].sample
      )
    end

    puts "Cadastrando Administradores...[OK]"
  end

  #Create Members
  desc "Cria Membros Fake"
  task generate_members: :environment do

    puts "Cadastrando Membros..."
    100.times do
      member = Member.new(
        email: Faker::Internet.email,
        password: "123456",
        password_confirmation: "123456"
      )
      member.build_profile_member
      member.profile_member.first_name = Faker::Name.first_name
      member.profile_member.second_name = Faker::Name.last_name

      member.save!
    end
    puts "Cadastrando Membros...[OK]"
  end

  #Create Ads
  desc "Cria Anuncios Fake"
  task generate_ads: :environment do

    puts "Cadastrando Anuncios..."

    5.times do
      Ad.create!(
        title: Faker::Lorem.sentence([2,3,4,5].sample),
        description_md: markdown_faker,
        description_short: Faker::Lorem.sentence([1,2].sample),
        member: Member.first,
        category: Category.all.sample,
        price: "#{Random.rand(500)},#{Random.rand(99)}",
        picture: File.new(Rails.root.join('public', 'templates', 'images-for-ads', "#{Random.rand(2..9)}.jpg"), 'r'),
        finish_date: Date.today + Random.rand(90)
      )
    end

    100.times do
      Ad.create!(
        title: Faker::Lorem.sentence([2,3,4,5].sample),
        description_md: markdown_faker,
        description_short: Faker::Lorem.sentence([1,2].sample),
        member: Member.all.sample,
        category: Category.all.sample,
        price: "#{Random.rand(500)},#{Random.rand(99)}",
        picture: File.new(Rails.root.join('public', 'templates', 'images-for-ads', "#{Random.rand(2..9)}.jpg"), 'r'),
        finish_date: Date.today() + Random.rand(90)

      )
    end
    puts "Cadastrando Anuncios...[OK]"
  end

   #Create Fake Comments
   desc "Cria Comentarios Fake"
   task generate_comments: :environment do
    
    puts "Cadastrando Comentarios..."
    Ad.all.each do |ad|
      (Random.rand(3)).times do
        Comment.create!(
          body: Faker::Lorem.paragraph([1..3].sample),
          member: Member.all.sample,
          ad: ad
        ) 
      end
   end
   puts "Cadastrando Comentarios...[OK]"
  end

  def markdown_faker
    %x(ruby -e "require 'doctor_ipsum'; puts DoctorIpsum::Markdown.entry")
  end
end